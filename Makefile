...
.PHONY: latex-common
latex-common:
	@echo "Editing Makefile to use XeLaTeX instead of pdfLaTeX..."
...
#		3. remove horizontal lines from tables
	@awk '\
		...
		1' \$(BUILDDIR)/latex/LassoGuide$(VERSION).tex | sed -E -e '/^(\\noindent\\begin\{tab|\\caption|\\\\$$|\{\{\\tablecontinued)/{N;s/\n\\hline//;}' \
		-e '/\{\{\\tablecontinued\{Continued on next page\}\}\}/{s/\|r\|/r/;s/\\hline$$//;}' \
...
latexpdf:
	...
	@echo "XeLaTeX finished; the PDF files are in $(BUILDDIR)."
...
latexpdfpb:
	...
	@echo "XeLaTeX finished; the PDF files are in $(BUILDDIR)/latex."
	@echo "Resave in Preview using Quartz Filter --> Gray Tone to create a grayscale PDF."
