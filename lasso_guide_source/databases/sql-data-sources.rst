...
-  Data sources can be case-sensitive. For best results, reference database and
   table names in the same lettercase as they appear on disk in your Lasso code.
   Field names may also be case sensitive (such as in Oracle and PostgreSQL).
...
.. method:: value_list(name::string)
   ...
   THis method executes an associated block once for each value allowed for an
